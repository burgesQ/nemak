...
(use-package general
  ...
  (nemacs/leader-keys
    ...
    "c o" '((lambda () (interactive)
              (find-file "~/repo/perso/nemacs_2/init.org"))
  ...
  ;; TODO: general go-mode
  ;; (nemacs/leader-keys go-mode-map)
  (nemacs/leader-keys ;; go-mode
    :major-modes '(go-mode)
    :keymap '(go-mode-map)
...
(use-package counsel
  ...
  :bind (("M-x" . counsel-M-x) ;; TODO: convert to  geneal
...
(use-package dashboard
  :ensure (:wait t)
  ;; :demand t
  :after (all-the-icons nerd-icons projectile page-break-lines s)
  :init
...
#+begin_src emacs-lisp
(defun nemacs/org-summary-todo (n-done n-not-done)
  "Switch entry to DONE when all subentries are done, to TODO otherwise."
  (let (org-log-done org-todo-log-states)   ; turn off logging
    (org-todo (if (= n-not-done 0) "DONE" "TODO"))))

(add-hook 'org-after-todo-statistics-hook #'nemacs/org-summary-todo)
#+end_src

Nicer checkbox.

#+BEGIN_SRC emacs-lisp
(defun nemacs/org-pretty-checkbox ()
...
#+BEGIN_SRC emacs-lisp
(defun nemacs/org-pretty-bullet ()
  ;; replace list '-' w/ a '•'
...
(use-package org
  ...
  (nemacs/org-better-stats)
  ;; (nemacs/org-pretty-checkbox) --> handled by org-modern
  (nemacs/org-pretty-bullet)
  (nemacs/org-better-def)
...
#+begin_src emacs-lisp
(defun nemacs/org--aggregate-cookies ()
  (save-excursion
    ...
    (let* ((prop (string-match "\\<aggregate\\>"
      ...
      (if prop
          (let* ((counts (nemacs/org--aggregate-cookies-one-level))
      ...
      (if (org-up-heading-safe)
          (nemacs/org--aggregate-cookies))
      )))

(defun nemacs/org--aggregate-cookies-one-level ()
  (save-excursion
    (let* ((current (point))
...
(add-hook 'org-checkbox-statistics-hook (function nemacs/org--aggregate-cookies))
#+end_src
...
(use-package lsp-mode
  ...
  ;; :demand t
  :hook (
         ;; (lsp-mode . nemacs/lsp-mode-setup)
  ...
  (setq
   lsp-ui-doc-enable t
   ;; disable file watched
   lsp-enable-file-watchers nil
   ;; don't keep workspace alive
   lsp-keep-workspace-alive nil
   ;; disable yasnippet
   lsp-enable-snippet nil
   ;; lsp-clangd
   ;; lsp-clangd-binary-path '/usr/bin/clangd
   ;; Use complation at point
  ...
  ;; :general FIXME: feed custom lsp general ? 
  ;; :keymap 'lsp-mode-map
  ;;   "l" '(:ignore t :wk "LSP")
  ;;   "l l" '(lsp-command-map :wk "LSP")
  ;;   "l f" '(:ignore t :wk "LSP find")
  ;;   "l fr" '(lsp-find-references t :wk "Reference")
  ;;   "l fr" '(lsp-treemacs-references t :wk "Tree reference")
  ;;   "l fd" '(lsp-find-definition t :wk "Definition")
  ;;   "l fD" '(lsp-find-declaration t :wk "Declaration")
  ;;   "l fi" '(lsp-find-implementation t :wk "Implementation")
  ;;   "l r" '(lsp-rename :wk "LSP rename")
  ;;   "l s" '(:ignore t :wk "LSP session")
  ;;   "l sd" '(lsp-describe-session :wk "LSP describe session")
  ;;   "l w" '(:ignore t :wk "LSP workspace")
  ;;   "l wa" '(lsp-workspace-folders-add :wk "LSP add workspace"))
)
#+end_src
...
(use-package go-mode
  ...
  :after general
  :hook
  (go-mode . nemacs/go-mode-setup)
  :config
  ;; (add-hook 'go-mode-hook #'nemacs/go-mode-setup)
...
(defun +nemacs/flycheck-checker-get(fn checker property)
  (or (alist-get property (alist-get checker flycheck-local-checkers))
      (funcall fn checker property)))
(advice-add 'flycheck-checker-get :around '+nemacs/flycheck-checker-get)
...
(add-hook 'after-change-major-mode-hook 'nemacs/flyspell-on-for-buffer-type)
...
(defun nemacs/flyspell-on-for-buffer-type ()
...
(defun nemacs/flyspell-toggle ()
  ...
  (if (symbol-value flyspell-mode)
    ...
    (nemacs/flyspell-on-for-buffer-type)))

(defun nemacs/fd-switch-dictionary()
  (interactive)
